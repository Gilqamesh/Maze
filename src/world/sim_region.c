...
void sim_region__routine(
    ...
    struct world_position start_world_p = world_position__from_relative_p(
        v3r32(-half_dims.x, -half_dims.y, -half_dims.z),
        center_p
    );
        ...
        for (i32 world_grid_x = start_world_p.global_p.x; world_grid_x <= end_world_p.global_p.x; ++world_grid_x) {
            for (i32 world_grid_z = start_world_p.global_p.z; world_grid_z <= end_world_p.global_p.z; ++world_grid_z) {
                ...
                for (u32 entity_index = 0; entity_index < grid->entities_size; ++entity_index) {
                    ...
                    if (entity != NULL) {
                        struct v3r32 entity_p = world_position__to_relative_p(entity->center_p, center_p);
                        entity->processor_callback_transient_values.relative_p = entity_p;
                        if (v3r32__is_in_half_dims(entity_p, half_dims)) {
                ...
                for (u32 entity_index = 0; entity_index < grid->entities_size; ++entity_index) {
                    ...
                    if (entity != NULL) {
                        ...
                        if (v3i32__is_equal(entity->center_p.global_p, grid_p) == false) {
